...
const cli = meow(
  ` Usage
    ...
    Examples
      $ abi-decoder --abi token.abi --input data.txt
      $ abi-decoder --abi token.abi "0x23b872dd..."
`,
  {
    flags: {
      abi: {
...
function run(abi, input) {
  ...
  for (let i = 0; i < result.types.length; i++) {
    ...
    output.push(
      `${result.types[i].padEnd(padType)}${result.names[i].padEnd(
        padName
      )}${value}`
    );
  }
